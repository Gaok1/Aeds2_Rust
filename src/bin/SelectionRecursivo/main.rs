...
//retorna o idx minimo
fn sort_rec(j : usize,min : usize, array: &mut Vec<Pokemon>) -> usize{
    if j == array.len(){

        return min;
    }
    if array[j].name < array[min].name{
        return sort_rec(j+1,j,array);
    }else{
        return sort_rec(j+1,min,array);
    }
