...
impl fmt::Display for Pokemon {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let types_str = self.types.iter().map(|t| format!("'{}'", t)).collect::<Vec<_>>().join(", ");
        let abilities_str = self.abilities.iter().map(|a| format!("'{}'", a)).collect::<Vec<_>>().join(", ");
...
fn main() {
    ...
    text_io::scan!("{}", entrada);
    //pesqusar os nomes por pesqusia binária
    while entrada.trim() != "FIM" {
        let name = entrada.trim();
        if bin_search(name, &pokemons) {
            println!("SIM");
        } else {
            println!("NAO");
        }
...
fn sort(array: &mut Vec<Pokemon>) {
    for i in 0..array.len() {
        ...
        for j in i + 1..array.len() {
            if array[j].name < array[min].name {
                min = j;
            }
        }
        if i != min {
            // Usa std::mem::swap para trocar os elementos
            array.swap(i, min);
        }
    }
}
...
fn bin_search(name: &str, pokemons: &Vec<Pokemon>) -> bool {
    let mut left = 0;
    let mut right = pokemons.len() -1 ;
    ...
    while left <= right {
        ...
        if name < &pokemons[index].name { //borrow por que o name é uma referencia e tem q ter mesmo tipo para fazer comparação
            //referencia compara com  referencia 
            right = index - 1;
        }else{
            left = index + 1;
        }
