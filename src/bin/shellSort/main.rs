use std::{
    cmp::Ordering, fmt, fs::File, io::{BufReader, Read}
};
...
fn main() {
    ...
    shell_sort(&mut pokemons); //ordenando
...
fn shell_sort(array: &mut Vec<Pokemon>) {
    let len = array.len();
    let mut gap = len / 2;
    ...
    while gap > 0 {
        for i in gap..len {
            ...
            // Mova os elementos que estão gap posições atrás para frente
            while j >= gap && array[j - gap].weight > val.weight {
                array[j] = array[j - gap].clone(); // Move o elemento
                j -= gap;
            }
