...
fn main() {
    ...
    insertion_sort(&mut pokemons); //ordenando
...
fn insertion_sort(array: &mut Vec<Pokemon>) {
    // Loop para iterar sobre cada elemento do vetor
    for i in 1..array.len() {
        let mut j = i;
        // Movendo o elemento para a posição correta
        while j > 0 && array[j-1].capture_date > array[j].capture_date {
            array.swap(j, j-1);
            j -= 1;
        }
    }
